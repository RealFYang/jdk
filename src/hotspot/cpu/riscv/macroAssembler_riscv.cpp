...
void MacroAssembler::safepoint_poll(Label& slow_path, bool at_return, bool acquire, bool in_nmethod, Register tmp_reg) {
  safepoint_poll(slow_path, fp, at_return, acquire, in_nmethod, tmp_reg);
}

void MacroAssembler::safepoint_poll(Label& slow_path, Register fp_reg, bool at_return, bool acquire, bool in_nmethod, Register tmp_reg) {
  assert_different_registers(fp_reg, tmp_reg);
  ld(tmp_reg, Address(xthread, JavaThread::polling_word_offset()));
  if (acquire) {
    membar(MacroAssembler::LoadLoad | MacroAssembler::LoadStore);
  }
  if (at_return) {
    bgtu(in_nmethod ? sp : fp_reg, tmp_reg, slow_path, /* is_far */ true);
  } else {
    test_bit(tmp_reg, tmp_reg, exact_log2(SafepointMechanism::poll_bit()));
    bnez(tmp_reg, slow_path, true /* is_far */);
  }
}
